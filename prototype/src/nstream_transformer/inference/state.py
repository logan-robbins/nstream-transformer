"""Role state management for the N-Stream inference runtime."""
...
@dataclass(slots=True)
class RoleState:
    """Tracks decode-time tensors and metadata for a single role lane."""
    ...
    def __post_init__(self) -> None:
        if self.input_ids.dim() != 2:
            raise ValueError("RoleState.input_ids must be rank-2 (batch, sequence).")
        if self.attention_mask.dim() != 2:
            raise ValueError("RoleState.attention_mask must be rank-2 (batch, sequence).")
        if self.input_ids.size() != self.attention_mask.size():
            raise ValueError("RoleState.input_ids and attention_mask must have identical shapes.")
        if self.input_ids.size(0) != 1:
            raise ValueError("RoleState expects batch size 1 during inference.")
        if self.commit_stride <= 0:
            raise ValueError("RoleState.commit_stride must be positive.")
        if self.commit_horizon <= 0:
            raise ValueError("RoleState.commit_horizon must be positive.")
        ...
        if self.commit_pointer:
            ...
            self._kv_checkpoints.append(
                KVCheckpoint(token_count=self.commit_pointer, past_key_values=_clone_cache(self.past_key_values))
            )
    ...
    def update_notes_window(self, notes: torch.Tensor, mask: Optional[torch.Tensor]) -> None:
        ...
        if notes.size(0) != 1:
            raise ValueError("RoleState only supports batch size 1 for notes windows.")
...
__all__ = ["PastKeyValues", "RoleState", "KVCheckpoint"]
